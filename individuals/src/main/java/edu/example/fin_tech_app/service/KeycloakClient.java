...
public class KeycloakClient {
        ...
        return webClient.post()
                ...
                .onStatus(status -> status.is4xxClientError() || status.is5xxServerError(),
                        clientResponse -> clientResponse.bodyToMono(String.class)
                                .flatMap(errorMessage -> Mono.error(new KeycloakIntegrationException("Unnable to get access_token: " + errorMessage,
    ...
    public Mono<AuthResponse> refreshToken(String refreshToken) {
        ...
        return webClient.post()
                ...
                .onStatus(status -> status.is4xxClientError() || status.is5xxServerError(),
                        clientResponse -> clientResponse.bodyToMono(String.class)
                                .flatMap(errorMessage -> Mono.error(new KeycloakIntegrationException("Unnable to refresh token: " + errorMessage,
                                        clientResponse.statusCode()
                                                .value()))))
                .bodyToMono(AuthResponse.class);
    }
                ...
                .onStatus(status -> status.is4xxClientError() || status.is5xxServerError(), resp ->
                        resp.bodyToMono(String.class)
                                .flatMap(errorMessage ->
                                        Mono.just(new KeycloakIntegrationException("Unnable to create user: " + errorMessage, resp.statusCode()
                                                .value()))))
    ...
    private Mono<AuthResponse> getAdminAccessToken() {
        ...
        return webClient.post().uri(uriBuilder -> uriBuilder.path(TOKEN_ENDPOINT).build(keycloakProps.getRealm()))
                ...
                .onStatus(status -> status.is4xxClientError() || status.is5xxServerError(), resp ->
                        resp.bodyToMono(String.class)
                                .flatMap(errorMessage ->
                                        Mono.just(new KeycloakIntegrationException("Unnabel to get admin token" + errorMessage, resp.statusCode()
                                                .value()))))
                .bodyToMono(AuthResponse.class);
    }
